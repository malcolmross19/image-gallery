...
let galleryContent = '';
const travel = document.querySelector('#travelHeader');
...
window.addEventListener('load', () => {
  ...
  request.onreadystatechange = function () {
    const responseObject = JSON.parse(request.responseText); // Parse the data from JSON

    for (let i = 0; i < responseObject.results.length; i + 1) {
...
travel.addEventListener('click', () => {
  const globeClasses = globe.classList;
